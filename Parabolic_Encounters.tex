...
The classix LISA design is a three arm, space-borne laser interferometer \citep{Bender1998, Danzmann2003}. The three arms form an equilateral triangle that rotates as the system's centre of mass follows a circular, heliocentric orbit, trailing $20^{\circ}$ behind the Earth. To describe the detector configuration, and to transform from the MBH coordinate system to those of the detector, we will find it useful to define three coordinate systems: those of the BH at the GC $x_\bullet^i$; ecliptic coordinates centred at the solar system (SS) barycentre $x_\odot^i$, and coordinates that co-rotate with the detector $x\sub{d}^i$. The MBH's coordinate system and the SS coordinate system are depicted in \figref{BH_SS}.
\begin{figure}
\begin{center}
...
\Figref{Examples} show example waveforms to demonstrate some of the possible variations in the signal. All these asssume the standard mass and position for the MBH as well as a $\mu = 10 M_\odot$ orbiting CO; other parameters are specified in the captions.
\begin{figure}
  \begin{center}
   \subfloat[]
    \label{fig:Orbit_11}
...
\begin{figure}
  \begin{center}
   ...
   %\subfloat[Detector II spectrum]{\resizebox{0.45\textwidth}{!}{%\import{./Images/}{h_II_5.tex}}} \\
    ...
    \label{fig:Orbit_13}
\caption{Example burst waveforms from the galactic centre. The strain $\widetilde{h}\sub{I}(f)$ is indicated by the solid line, $\widetilde{h}\sub{II}(f)$ is indicated by the dot-dashed line. The LISA noise curve is indicated by the dashed line. The kludge has been formulated using oblate spheroidal coordinates.}
  \end{center}
\label{fig:Examples}
\end{figure}
...
elseif orbit == 11 % One loop SNRo = 1.5638e+04, SNRs = 1.4137e+04.
    P_BH(1) = 4.31e6*M_sun; % BH mass in kg
    P_BH(2) = 0.9; % Specific angular momentum
    P_BH(3) = pi/2; % Polar angle from spin axis
    P_BH(4) = 0; % Azimuthal angle without loss of generality
    P_BH(5) = pi/4; % Psi
    P_BH(6) = 2.57e20; % Solar system-galactic centre distance in m
    P_BH(7) = 95.607669 * pi/180;
    P_BH(8) = 266.851760 * pi/180;
    
    m2s_1_M_BH = c_light / (G_grav * P_BH(1)); % metres^2/second to BH mass
      
    flat_peri = 0;
    
        [L_zs Q_Cs] = Orbit_angular_momenta(3.5, 0.05, 1, P_BH(2));

        [L_zo Q_Co] = Orbit_angular_momenta(sqrt(3.5^2-P_BH(2)^2), 0.05, 1, P_BH(2));
    
    
    theta = 1.56;
    phi = pi/4;
    
    P_orbit(1) = L_zs(1) / m2s_1_M_BH; % Specific angular momentum about BH's z-axis 6.5e18!!!
    P_orbit(2) = Q_Cs(1) / m2s_1_M_BH^2; % Carter's constant per unit mass squared
    P_orbit(3) = 10*M_sun; % Mass of orbitting body in kg
    P_orbit(4) = 0; % Starting coordinates for orbit in BH's coordinates, time set to zero without loss of generality
    P_orbit(5) = 100e11*sin(theta)*cos(phi); % x
    P_orbit(6) = 100e11*sin(theta)*sin(phi); % y
    P_orbit(7) = 100e11*cos(theta); % z
elseif orbit == 12 % One large loop SNRo = 97.2716 SNRs = 97.1458
    P_BH(1) = 4.31e6*M_sun; % BH mass in kg
    P_BH(2) = 0.5; % Specific angular momentum
    P_BH(3) = 0; % Polar angle from spin axis
    P_BH(4) = 0; % Azimuthal angle without loss of generality
    P_BH(5) = 0; % Psi
    P_BH(6) = 2.57e20; % Solar system-galactic centre distance in m
    P_BH(7) = 95.607669 * pi/180;
    ...
    [L_z Q_C] = Orbit_angular_momenta(20, 1.5, 1, P_BH(2));
    
    theta = pi/6;
    phi = pi/4;
    
    P_orbit(1) = L_z(1) / m2s_1_M_BH; % Specific angular momentum about BH's z-axis 6.5e18!!!
    P_orbit(2) = Q_C(1) / m2s_1_M_BH^2; % Carter's constant per unit mass squared
    P_orbit(3) = 10*M_sun; % Mass of orbitting body in kg
    P_orbit(4) = 0; % Starting coordinates for orbit in BH's coordinates, time set to zero without loss of generality
    P_orbit(5) = 100e11*sin(theta)*cos(phi); % x
    P_orbit(6) = 100e11*sin(theta)*sin(phi); % y
    P_orbit(7) = 100e11*cos(theta); % z
elseif orbit == 13 % Dull SNRo = 10.7168 SNRs = 10.7157
    P_BH(1) = 4.31e6*M_sun; % BH mass in kg
    P_BH(2) = 0.3; % Specific angular momentum
    P_BH(3) = pi/3; % Polar angle from spin axis
    P_BH(4) = 0; % Azimuthal angle without loss of generality
    P_BH(5) = pi/6; % Psi
    P_BH(6) = 2.57e20; % Solar system-galactic centre distance in m
    P_BH(7) = 95.607669 * pi/180;
    ...
    [L_z Q_C] = Orbit_angular_momenta(50, 0, 1, P_BH(2));
...
\begin{figure*}
  \begin{center}
   \subfloat[MBH mass $M_\bullet$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_1}} \quad
   \subfloat[MBH spin $a_\ast$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_2}} \\
   \subfloat[Propagation angle $\Theta$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_3}} \quad
   \subfloat[Orientation angle $\Psi$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_4}} \\
   \subfloat[Distance $\ln R_0$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_5}} \quad
   \subfloat[MBH direction $\overline{\Theta}$]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_6}}
...
\ContinuedFloat
  \begin{center}
   \subfloat[MBH direction $\overline{\Phi}$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_7}} \quad
   \subfloat[Orbital angular momentum $L_z$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_8}} \\
   \subfloat[Carter constant $Q$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_9}} \quad
   \subfloat[CO mass $\mu$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_10}} \\
   \subfloat[Initial position $x_0$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_11}} \quad
   \subfloat[Initial position $y_0$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_12}}
   \caption{Mass-normalised parameter uncertainties and SNR. The points represent the entire set of data. The lines are maximum likelihood fits to the values obtained by averaging over each set of intrinsic parameters. See \tabref{lines} for best fit parameters.}
    \label{fig:sigmas}
  \end{center}
\end{figure*}
...
\ContinuedFloat
  \begin{center}
	\subfloat[Initial position $z_0$.]{\includegraphics[width=0.45\textwidth]{Fig_SNR_sigma_13}}
	\caption{Mass-normalised parameter uncertainties and SNR. The points represent the entire set of data. The lines are maximum likelihood fits to the values obtained by averaging over each set of intrinsic parameters. See \tabref{lines} for best fit parameters.}
    \label{fig:sigmas}
  \end{center}
\end{figure}
...
\begin{figure*}
  \begin{center}
   \subfloat[$r\sub{p} = 7.5 r\sub{S}$, log-log plot.]{\includegraphics[width=0.45\textwidth]{Fig_Loglog_E_15}} \quad
   \subfloat[$r\sub{p} = 7.5 r\sub{S}$, log-linear plot.]{\includegraphics[width=0.45\textwidth]{Fig_Loglin_E_15}} \\
   \subfloat[$r\sub{p} = 15.0 r\sub{S}$, log-log plot.]{\includegraphics[width=0.45\textwidth]{Fig_Loglog_E_30}} \quad
   \subfloat[$r\sub{p} = 15.0 r\sub{S}$, log-linear plot.]{\includegraphics[width=0.45\textwidth]{Fig_Loglin_E_30}} \\
   \subfloat[$r\sub{p} = 30.0 r\sub{S}$, log-log plot.]{\includegraphics[width=0.45\textwidth]{Fig_Loglog_E_60}} \quad
   \subfloat[$r\sub{p} = 30.0 r\sub{S}$, log-linear plot.]{\includegraphics[width=0.45\textwidth]{Fig_Loglin_E_60}}
...
\begin{figure*}
  \begin{center}
   \subfloat[]{\includegraphics[width=0.45\textwidth]{Fig_Energy_ratio_peri}} \quad
   \subfloat[]{\includegraphics[width=0.45\textwidth]{Fig_Energy_ratio_orbit}}
...
\begin{figure*}
  \begin{center}
   \subfloat[Spectrum using no window. The calculated SNR is $\rho = 12.5$.]{\label{fig:Windowing_Rectangular} \includegraphics[width=0.45\textwidth]{Fig_h_I_9_rectangular}} \quad
   \subfloat[Spectrum using a Nuttall window. The calculated SNR is $\rho = 8.5$.]{\label{fig:Windowing_Nuttall} \includegraphics[width=0.45\textwidth]{Fig_h_I_9_Nuttall}}
