...
\begin{figure}
  ...
  \includegraphics[width=0.45\textwidth]{Fig_SNR}
    \caption{Mass-normalised SNR as a function of periapse radius. The plotted points are the values obtained by averaging over each set of intrinsic parameters. The best fit line is $\log(\hat{\rho}/M_\odot^{-1}) = -2.64\log(r\sub{p}/r\sub{g}) + 4.78$. This is fitted to orbits with $r\sub{p} >  11.66 r\sub{g}$ and has a reduced chi-squared value of $\chi^2/\nu = 1.47$.}
    \label{fig:SNR}
  \end{center}
\end{figure}
Closer orbits produce louder bursts. To reflect this relationship, we have fitted a simple fiducial power law, as indicated by the straight line. This was done by maximising the likelihood, assuming that $\ln \rho$ has a Gaussian distribution with standard deviation derived from the scatter of points due to variation in the intrinsic parameters. A power law appears to be a good fit only for orbits with larger periapses. At smaller periapses, corresponding to more extreme relativistic orbits, the relationship adopts a more complicated shape that cannot be well approximated by a simple power law. We only used orbits with periapses larger than a cut-off $r\sub{p} > r\sub{cut}$ for the fit. We chose $r\sub{cut} = 2(1 + \sqrt{2})^2 r\sub{g} \simeq 11.66 r\sub{g}$ as a cut-off; this is twice the largest value for the innermost non-plunging equatorial orbit (corresponding to a retrograde orbit about a maximally spinning MBH). This choice is arbitray, but should exclude the most severe relativistic effects. Changing the cut-off within a plausible region alters the fit coefficients in the last significant figure. since we are only inerested in a rough trend, we shall be concerned by this.
...
\begin{figure}
  ...
  \includegraphics[width=0.45\textwidth]{Fig_SNR_M32}
    \caption{Signal-to-noise ratio as a function of periapse radius for a $\mu = 1 M_\odot$ compact object about the MBH of M32. The plotted points are the values obtained by averaging over each set of intrinsic parameters. The best fit line is $\log\left(\rho\right) = -2.41\log(r\sub{p}/r\sub{g}) + 2.71$. This is fitted to orbits with $r\sub{p} > 11.66 r\sub{g}$ and has a reduced chi-squared value of $\chi^2/\nu = 1.02$.}
    \label{fig:SNR-M32}
  \end{center}
\end{figure}
We see that the general behaviour is the same as for the GC, but there are differences because of the position. Bursts can be detected if the periapse is smaller than $r\sub{p} \simeq X r\sub{g} \simeq X\units{pc}$. These galaxies are at a distance of $770\units{kpc}$ \citep{Karachentsev2004}. It therefore seems unlikely that bursts could be observed from the Virgo cluster at a distance of $R\sub{Virgo} \approx 16.5\units{Mpc}$ \citep{Mei2007}.
...
The signal-to-noise ratio of bursts is well correlated with the periapsis. Except for the closest orbits ($r\sub{p} \lesssim 7 r\sub{g}$), the SNR (per unit mass) may be described as having a power-law dependence of
\begin{equation}
\log\left(\hat{\rho}/M_\odot^{-1}\right) = -2.64\log(r\sub{p}/r\sub{g}) + 4.78.
\end{equation}
