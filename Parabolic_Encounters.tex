...
\begin{equation}
\ln r = -\recip{2}\innerprod{\Delta\lambda^a\partial_a\boldsymbol{h}\sub{ML} - \Delta\bodsymbol{h}}{\Delta\lambda^b\partial_b\boldsymbol{h}\sub{ML} - \Delta\bodsymbol{h}}.
\end{equation}
...
The authors are indebted to Michele Vallisneri for useful discussions on the proper use of Fisher matrices, and are grateful to Dave Green for helpful suggestions regarding apodization. They would also like to thank Donald Lynden-Bell for useful suggestions. CPLB is supported by STFC. JRG is supported by the Royal Society.
