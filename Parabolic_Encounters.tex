...
Markov chain Monte Carlo (MCMC) methods are widely used for inference problems; they are a family of algorithms used for sampling an unknown probability distribution and are efficient for high-dimensional problems~\citep[chapter 29]{MacKay2003}. Parameter space is explored by constructing randomly a chain of $N$ samples. The distribution of points visited by the chain maps out the underlying distribution; this becomes asymptotically exact as $N \rightarrow \infty$. Samples are added sequentially, if the current state is $\boldsymbol{lambda}_i$ a new point $\boldsymbol{lambda}^\ast$ is drawn and accepted with probability
\begin{equation}
R = \min\left\{\frac{\mathcal{L}(\boldsymbol{lambda}^\ast)Q(\boldsymbol{lambda}^\ast)}{\mathcal{L}(\boldsymbol{lambda}_i)Q(\boldsymbol{lambda}_i)}, 1\right\},
\end{equation}
setting $\boldsymbol{lambda}_{i + 1} = \boldsymbol{lambda}^\ast$, where is the \mathcal{L} likelihood, calculated in our case from \eqnref{}, and $Q$ is a proposal distribution. If the move is not accepted we set $\boldsymbol{lambda}_{i + 1} = \boldsymbol{lambda}_i$.

Simply waiting long enough will yield an exact posterior. However, we are impatient, and it is desirable for the MCMC to converge quickly. This requires a suitable choice for the proposal distribution. This is difficult to define, since we do not know ahead of time the shape of the target distribution: ideally they would be identical.
