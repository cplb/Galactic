...
We take the mean of $\ln \sigma$ for each set of 20 randomised parameters (starting position, MBH orientation and LISA orientation).\footnote{The logarithm is a better quantity to work with since the standard deviation is a scale parameter that may be distributed over a range of magnitudes (\citealt{Mackay2003}, chapter 22). Using the median value for $\sigma$ yields results that are similar quantitatively.}

There exists a correlation between the periapse radius and SNR, as shown in \figref{SNR}. Closer orbits produce louder bursts. The straight line is a maximum likelihood fit assuming that the SNR has a Gaussian distribution with standard deviation derived from the scatter of points. The parameter uncertainties are well correlated with the SNR. Some are sensitive to other parameters, such as the MBH spin, or the orbital inclination.
...
\end{equation}
and functions $A\left(\widetilde{f}\right)$ and $B\left(\widetilde{f}\right)$ are defined in terms of Bessel functions. Their precise forms are \citep{Berry2010}
\begin{align}
A\left(\widetilde{f}\right) = {} & \recip{\pi}\sqrt{\frac{2}{3}}K_{1/3}\left(\frac{2^{3/2}\widetilde{f}}{3}\right); \\
B\left(\widetilde{f}\right) = {} & \recip{\sqrt{3}\pi}\left[K_{-2/3}\left(\frac{2^{3/2}\widetilde{f}}{3}\right) + K_{4/3}\left(\frac{2^{3/2}\widetilde{f}}{3}\right) \right. \nonumber \\
...
\begin{figure*}
  ...
  \end{center}
\end{figure*}
We introduce an additional energy here, the quadrupole NK energy $E\sub{NK(Q)}$. This can be calculated in three ways:
\begin{enumerate}
...
\label{eq:E_quad}
\end{equation}
where $\Ibar_{ij} = I_{ij} - (1/3)I\delta_{ij}$ is the reduced mass quadrupole tensor. This can be obtained from \eqnref{integrate_E} by integrating over all angles when the waveform only contains the mass quadrupole component. This has the advantage of avoiding the effects of spectral leakage or the influence of window functions.
\item Using the analytic expressions for the integral \eqnref{E_quad} given in appendix A of \citet{Gair2005}.
\end{enumerate}
All three agree to within computational error. This demonstrates the validity of the code, and shows that the use of a window function does not significantly distrort the waveform.
...
From the level of agreement we may be confident that the NK waveforms are a reasonable approximation. The difference in energy flux is only greater than $10\%$ for very strong fields $r\sub{p} \simeq 2 r\sun{S}$; since this is dependent on the square of the waveform, typical accuracy in the waveform may be $\sim 5\%$ \citep{Gair2005,Tanaka1993}.
...
\begin{figure*}
  \begin{center}
   ...
   \subfigure[Spectrum using a Nuttall window. The calculated SNR is $\rho = 8.5$.]{\label{fig:Windowing_Nuttal} \includegraphics[width=0.45\textwidth]{Fig_h_I_9_Nuttall}}
