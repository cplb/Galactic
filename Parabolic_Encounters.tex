...
Having constructed the GW $\boldsymbol{h}(t)$ that shall be incident upon the detector, we may now consider how to analyse the waveform and extract the information it contains. We begin with a brief overview of the basic components of signal analysis used for GWs, with application to \textit{LISA} in particular. This fixes notation. A more complete discussion can be found in \citet{Finn1992}, and \citet{Cutler1994}. Adaption for \textit{eLISA} requires a substitution of the noise distribution, and the removal of the sum over detectors, since it will only have one.
...
\end{equation}
The functional form of the noise spectral density $S_n(f)$ for \textit{LISA} is discussed in \secref{Noise}.
...
As a consistency check, to verify that the mismatch between the Fisher matrix and MCMC results is a consequence of parameter correlations, we calculated one-dimensional Fisher matrices, only varying the MBH mass, and compared these to widths computed from MCMCs only sampling in mass. These were found to be in good agreement. The majority ($\sim 62\%$) have standard deviations consistent to within a factor of two. Approximately $4\%$ differed by more than an order of magnitude: these were exactly the orbits that failed to fulfill the maximum-mismatch criterion.
...
\begin{figure*}
  \begin{center}
   ...
   \subfigure[Spectrum using a Nuttall window. The calculated SNR is $\rho = 8.5$.]{\label{fig:Windowing_Nuttall} \includegraphics[width=0.45\textwidth]{Fig_h_I_9_Nuttall}}
    \caption{Example spectra calculated using (a) a rectangular window and (b) Nuttall's four-term window with continuous first derivative \citep{Nuttall1981}. The spin of the MBH is $a_\ast = 0.5$, the mass of the orbiting CO is $\mu = 10 M_\odot$, the periapsis is $r\sub{p} = 50 r\sub{g}$ and the inclination is $\iota = 0.1$. The high-frequency tail is the result of spectral leakage. The level of the \textit{LISA} noise curve is indicated by the dashed line. The spectra are from detector I, detector II has similar spectra.}
    \label{fig:Windowing}
  \end{center}
\end{figure*}
