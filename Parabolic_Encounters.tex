...
\begin{figure}
  ...
  \end{center}
\end{figure}
Closer orbits produce louder bursts. To reflect this relationship, we have fitted a simple fiducial power law, as indicated by the straight line. This was done by maximising the likelihood, assuming that $\ln \rho$ has a Gaussian distribution with standard deviation derived from the scatter of points due to variation in the intrinsic parameters. A power law appears to be a good fit only for orbits with larger periapses. Smaller periapses correspond to higher frequencies: the change in the trend reflect the change of the noise curve as we go from approximately power law behaviour into the bucket of the curve. We fit a pwer law to orbits with a characteristic frequency of ?.\footnote{The power law exponent $-2.bit$ is not consistent with $-13/4$ as predicted by the approximate model of \citet{Hopman}.} Changing the cut-off within a plausible region alters the fit coefficients in the last significant figure. Since we are only inerested in a rough trend, we shall be concerned by this.
...
\begin{figure}
  ...
  \end{center}
\end{figure}
We see that the general behaviour is the same as for the GC, but there are differences because of the position. Bursts for a $1 M_\odot$ ($10 M_\odot$) can be detected if the periapse is smaller than $7 r\sub{g}$ ($13 r\sub{g}$). These galaxies are at a distance of $770\units{kpc}$ \citep{Karachentsev2004}. It therefore seems unlikely that bursts could be observed from the Virgo cluster at a distance of $R\sub{Virgo} \approx 16.5\units{Mpc}$ \citep{Mei2007}.
