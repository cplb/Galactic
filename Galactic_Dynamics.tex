...
\end{equation}
where $\rho_\star$ is the mass density, the Coulomb logarithm is $\ln\Lambda = \ln(M_\bullet/M_\star)$, and $\kappa$ is a dimensionless number. In his pioneering work, \citet[chapter 2]{Chandrasekhar1960} defined the time-scale as the period over which the squared change in energy was equal to the kinetic energy squared, this gives $\kappa = 9/16\sqrt{\pi} \simeq 0.32$. \citet{Bahcall1977} define a reference time-scale from their Boltzmann equation with $\kappa = 3/4\sqrt{8\pi} \simeq 0.15$; this is equal to the reference time-scale defined as the reciprocal of the coefficient of dynamical friction by \citet{Chandrasekhar1943}. \citet{Spitzer1958} define a reference time-scale from the gravitational Boltzmann equation of \citet{Spitzer1951} where $\kappa = \sqrt{2}/\pi \simeq 0.45$. Following \citep{Spitzer1971}, \citet[section 8.3.4]{Binney1987} estimate the time-scale from the velocity diffusion coefficient of the Fokker-Planck equation yielding $\kappa \simeq 0.34$.
...
Stars orbiting about the black hole will continually emit gravitational radiation, this carries away energy and angular momentum, causing the stars to inspiral. Using the analysis of \citet{Peters1964} for Keplerian binaries it is possible to define a characteristic inspiral time-scale from the rate of change of energy: we will assume that an orbit is depleted of stars if the characteristic inspiral time-scale is shorter than the relaxation time-scale. For consistency with the definition of the relaxation time-scale, we will define the characteristic inspiral timescale as...

For comparison, the total time taken for the inspiral is given in \eqnref{Bound_DF}. The characteristic time-scale is a better measure of the depletion of an orbit as it only depends upon the parameters of that orbit, and not its future evolution.
...
\end{align}
the former piece still contains the logarithmic term which we know must be large. It is therefore the dominant piece of the integral and we neglect the latter,
\begin{equation}
...
\end{equation}
This may be further simplified, reusing the limit of large $q$,
\begin{align}
...
\frac{v_2^2}{v_1}\ln\left(\frac{v_1 + v_2}{v_2 - v_1}\right) + \frac{v_1^2}{v_2}\left[\ln qv_2^2 + \ln\left(1 - \frac{v_1^2}{v_2^2}\right)\right] & v_1 < v_2 \\
v_2\left(\ln qv_2^2 + \ln 4\right) & v_1 = v_2\\
\frac{v_1^2}{v_2}\ln\left(\frac{v_1 + v_2}{v_1 - v_2}\right) + \frac{v_2^2}{v_1}\left[\ln qv_2^2 + \ln\left(\frac{v_1^2}{v_2^2} - 1\right)\right] & v_1 > v_2
\end{cases} \\
\approx {} & \frac{8}{3}\begin{cases}
\frac{v_1^2}{v_2}\ln qv_2^2 & v_1 \leq v_2 \\
\frac{v_2^2}{v_1}\ln qv_2^2 & v_1 \geq v_2
\end{cases}.
\end{align}
This form maintains the correct limit for $v_2 \rightarrow 0$. We are left with
\begin{equation}
\Delta E^2(v_1) \approx \frac{8\pi}{3} n(v_1)G^2m_1^2 m_2^2\ln qv_2^2\begin{Bmatrix}\frac{v_1^2}{v_2} & v_1 \leq v_2\\ \frac{v_2^2}{v_1} & v_1 \geq v_2 \end{Bmatrix}\,\dd v_1\delta t.
\end{equation}
...
\intd{-\infty}{w-1}{\frac{(-x)^{p_M}}{(w-x)^4}}{x} & w \leq 1 \\
\intd{-\infty}{0}{\frac{(-x)^{p_M}}{(w-x)^4}}{x} & w \geq 1
\end{cases}.
\end{equation}
...
\begin{equation}
f_{\mathrm{b},\,M}(v) = \frac{N_\ast}{\left(2\pi\sigma^2\right)^{3/2}}k_M \left(\frac{v^2}{2\sigma^2}\right)\begin{cases}
3 \Beta\left(\frac{v^2}{2\sigma^2};3 - p_M, 1 + p_M\right) & \frac{v^2}{2\sigma^2} \leq 1 \\
\frac{\Gamma(3 - p_M)\Gamma(1 + p_M)}{2} & \frac{v^2}{2\sigma^2} \geq 1
\end{cases},
\end{equation}
where $\Beta(x;a,b)$ is the incomplete beta function (\citealt{Olver2010}, 8.17), and $\Gamma(x)$ is the gamma function.
...
\begin{equation}
\Delta E^2 \approx \frac{8}{3}\sqrt{2\pi}\frac{G^2m_1^2 m_2^2n_\ast}{\sigma^3}\ln qv_2^2\left\{\intd{0}{v_2}{3k\frac{v_1^4}{v_2}\left(\frac{v_1^2}{2\sigma^2}\right)^{p-3} \Beta\left(\frac{v_1^2}{2\sigma^2};3 - p, 1 + p\right)}{v_1} + \intd{v_2}{\sqrt{2}\sigma}{3kv_1v_2^2\left(\frac{v_1^2}{2\sigma^2}\right)^{p-3} \Beta\left(\frac{v_1^2}{2\sigma^2};3 - p, 1 + p\right)}{v_1} + \intd{\sqrt{2}\sigma}{\infty}{v_1v_2^2\left[k\left(\frac{v_1^2}{2\sigma^2}\right)^{p-3}\frac{\Gamma(3 - p_M)\Gamma(1 + p_M)}{2} + C\epsilon\left(\frac{v_1^2}{2\sigma^2}\right)\right]}{v_1}\right\}\,\delta t,
\end{equation}
where we have suppressed the subscript $M$ for brevity; it would be necessary to sum over all the species to get the total value. Computing the integrals yields
\begin{equation}
\Delta E^2 \approx \frac{16}{3}\sqrt{2\pi}\frac{G^2m_1^2 m_2^2n_\ast}{\sigma^3}\ln qv_2^2 \frac{v_2^2}{2\sigma^2} \left\{k\left[\frac{12}{5(1 - 2p)}\frac{v_2^2}{2\sigma^2} {_2F_1}\left(\frac{5}{2},-p;\frac{7}{2};\frac{v_2^2}{2\sigma^2}\right) -\frac{9}{(2-p)(1-2p)}\left(\frac{v_2^2}{2\sigma^2}\right)^{p-2}\Beta\left(\frac{v_2^2}{2\sigma^2};3-p,1-p\right) + \frac{3}{(2-p)(1+p)}\left(1- \frac{v_2^2}{2\sigma^2}\right)^(1+p)\right] + C\right\}\,\delta t,
\end{equation}
assuming that $p < 1/2$. The contribution from bound and unbound stars can be identified by the coefficients $k$ and $C$ respectively.

If $v_2^2/2\sigma^2 > 1$,
\begin{equation}
\Delta E^2 \approx \frac{8}{3}\sqrt{2\pi}\frac{G^2m_1^2 m_2^2n_\ast}{\sigma^3}\ln qv_2^2\left\{\intd{0}{\sqrt{2}\sigma}{3k\frac{v_1^4}{v_2}\left(\frac{v_1^2}{2\sigma^2}\right)^{p-3} \Beta\left(\frac{v_1^2}{2\sigma^2};3 - p, 1 + p\right)}{v_1} + \intd{\sqrt{2}\sigma}{v_2}{\frac{v_1^4}{v_2}\left[k\left(\frac{v_1^2}{2\sigma^2}\right)^{p-3}\frac{\Gamma(3 - p_M)\Gamma(1 + p_M)}{2} + C \epsilon\left(\frac{v_1^2}{2\sigma^2}\right)\right]}{v_1} + \intd{v_2}{\infty}{v_1v_2^2\left[k\left(\frac{v_1^2}{2\sigma^2}\right)^{p-3}\frac{\Gamma(3 - p)\Gamma(1 + p)}{2} + C \epsilon\left(\frac{v_1^2}{2\sigma^2}\right)\right]}{v_1}\right\}\,\delta t.
\end{equation}
Computing this gives
\begin{equation}
\Delta E^2 \approx \frac{16}{3}\sqrt{2\pi}\frac{G^2m_1^2 m_2^2n_\ast}{\sigma^3}\ln qv_2^2 \left(\frac{v_2^2}{2\sigma^2}\right)^{-1/2} \left\{k\left[\frac{9\sqrt{\pi}\Gamma(1+p)}{2(1-2p)\Gamma(p + 7/2)} - \frac{3\Gamma(3 - p)\Gamma(1 + p)}{2(1-2p)(2-p)}\left(\frac{v_2^2}{2\sigma^2}\right)^{p-1/2}\right] + C\alpha\left(\frac{v_2^2}{2\sigma^2}\right)\right\}\,\delta t,
\end{equation}
where
\begin{equation}
\alpha(w) = \recip{2}\left\{3w^{-1/2} + 5 + \left[4\exp(1) - \Ei(1) + \Ei(w)\right]\left[\frac{3\sqrt{\pi}}{4}\erf\left(w^{1/2}\right) - \frac{3}{2}w^{1/2}\exp(-w)\right] - 3\sqrt{\pi}\exp(1)\erf(1) + 3\left[{_2F_2}\left(\frac{1}{2},1;\frac{3}{2},\frac{3}{2};1\right) - w^{1/2}{_2F_2}\left(\frac{1}{2},1;\frac{3}{2},\frac{3}{2};w\right)\right]\right\}.
\end{equation}
...
Combining the two regimes gives some quite complicated expressions. It is possible to simplify these while maintaining reasonable accuracy, for this purpose we will separate the contributions from bound and unbound stars. The bound contribution is relatively straightforward for larger $v_2$, it is just the sum of two power laws, the relative importance of each depends upon the value of $p$. For smaller $v_2$ we may use a series expansion in $v_2^2/2\sigma^2 < 1$ to approximate the special functions:
\begin{align}
\Delta E_b^2 \approx {} & 16\sqrt{2\pi}G^2m_1^2m_2^2n_\ast\sigma\ln qv_2^2 k \begin{Bmatrix}
\frac{3}{(1 + p)(2 - p)} - \frac{9}{5(3-p)}\frac{v_2^2}{2\sigma^2} + \frac{9p}{14(7-p)}\left(\frac{v_2^2}{2\sigma^2}\right)^2 + \frac{p(1-p)}{6(5-p)}\left(\frac{v_2^2}{2\sigma^2}\right)^3 & \frac{v_2^2}{2\sigma^2} < 1 \\
\recip{1 - 2p}\left(\frac{v_2^2}{2\sigma^2}\right)^{-1/2}\left[\frac{9\sqrt{\pi}\Gamma(1+p)}{2\Gamma(p + 7/2)} - \frac{3\Gamma(3 - p)\Gamma(1 + p)}{2(2-p)}\left(\frac{v_2^2}{2\sigma^2}\right)^{p-1/2}\right] & \frac{v_2^2}{2\sigma^2} > 1\end{Bmatrix}\,\delta t.
\end{align}
This can be rolled into one continuous function
\begin{align}
\Delta E_b^2 \approx {} & 16\sqrt{2\pi}G^2m_1^2m_2^2n_\ast\sigma\ln qv_2^2 k \left\{\left[1 + \left(\frac{v_2^2}{2\sigma^2}\right)^4\right]^{-1}\left[\frac{3}{(1 + p)(2 - p)} - \frac{9}{5(3-p)}\frac{v_2^2}{2\sigma^2} + \frac{9p}{14(7-p)}\left(\frac{v_2^2}{2\sigma^2}\right)^2 + \frac{p(1-p)}{6(5-p)}\left(\frac{v_2^2}{2\sigma^2}\right)^3 \right] + \recip{1 - 2p}\left(\frac{v_2^2}{2\sigma^2}\right)^{7/2}\left[1 + \left(\frac{v_2^2}{2\sigma^2}\right)^4\right]^{-1}\left[\frac{9\sqrt{\pi}\Gamma(1+p)}{2\Gamma(p + 7/2)} - \frac{3\Gamma(3 - p)\Gamma(1 + p)}{2(2-p)}\left(\frac{v_2^2}{2\sigma^2}\right)^{p-1/2}\right]\right\}\,\delta t\\
 \approx {} & 6\sqrt{2\pi}G^2m_1^2m_2^2n_\ast\sigma\ln qv_2^2 k \beta\left(\frac{v_2^2}{2\sigma^2};p\right)\,\delta t,
\end{align}
defining the function $\beta(w;p)$ in the last line. The resulting error (ignoring variation from $\ln qv_2^2$) is less than $3\%$.
...
\begin{align}
\lim_{w \rightarrow \infty}\left\{\alpha(w)\right\} = {} & \recip{2}\left\{5 + \frac{3\sqrt{\pi}}{4}\left[4\exp(1) - \Ei(1) - 4\exp(1)\erf(1)\right] + 3{_2F_2}\left(\frac{1}{2},1;\frac{3}{2},\frac{3}{2};1\right)\right\} \\
 = {} & \Xi.
\end{align}
Using the two limit forms, the function
\begin{equation}
\Delta E_u^2 \approx 16\sqrt{2\pi}G^2m_1^2m_2^2n_\ast\sigma\ln qv_2^2 C \Xi\frac{v_2^2}{2\sigma^2}\left[\Xi^2 + \left(\frac{v_2^2}{2\sigma^2}^3\right)^4\right]^{-1/2}\,\delta t,
\end{equation}
reproduces the full function to better than $5\%$ (ignoring variation from $\ln qv_2^2$).

Making explicit the sum over the different species, the total change in energy squared is approximately
\begin{equation}
\Delta E^2 \approx \sum_M 16\sqrt{2\pi}G^2m_M^2m_2^2n_\ast\sigma\ln qv_2^2 \left\{k_M \beta\left(\frac{v_2^2}{2\sigma^2};p_M\right) + C_M\Xi\frac{v_2^2}{2\sigma^2}\left[\Xi^2 + \left(\frac{v_2^2}{2\sigma^2}^3\right)^4\right]^{-1/2}\right\}\,\delta t.
\end{equation}
...
\tau\sub{R} = {} & \left(\frac{m_2v_2^2}{2}\right)^2\frac{\delta t}{\Delta E^2} \\
 \approx {} & \frac{v_2^4}{16\sqrt{2\pi}G^2n_\ast\sigma\ln qv_2^2} \left(\sum_M m_M^2 \left\{k_M \beta\left(\frac{v_2^2}{2\sigma^2};p_M\right) + C_M\Xi\frac{v_2^2}{2\sigma^2}\left[\Xi^2 + \left(\frac{v_2^2}{2\sigma^2}^3\right)^4\right]^{-1/2}\right\}\right)^{-1}.
\end{align}
...
\begin{equation}
\tau(r\sub{p0},e_0) = \intd{0}{e_0}{\frac{15}{304}\frac{\chi^4}{\zeta}\frac{e^{29/19}}{(1-e^2)^{3/2}}\left(1 + \frac{121}{304}e^2\right)^{1181/2299}}{e}.
\end{equation}
