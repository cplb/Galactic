...
\begin{equation}
\Delta E^2 \approx \frac{16}{3}\sqrt{2\pi}\frac{G^2m_1^2 m_2^2n_\ast}{\sigma^3}\ln qv_2^2 \left(\frac{v_2^2}{2\sigma^2}\right)^{-1/2} \left\[k\beta\left(\frac{v_2^2}{2\sigma^2};p\right) + C\alpha\left(\frac{v_2^2}{2\sigma^2}\right)\right\]\,\delta t,
\end{equation}
where
\begin{align}
\beta(w;p) = {} & \begin{cases} \left[\dfrac{9\sqrt{\pi}\Gamma(1+p)}{2(1-2p)\Gamma(p + 7/2)} - \dfrac{9\Beta\left(3 - p, 1 + p\right)}{(1-2p)(2-p)}\left(\dfrac{v_2^2}{2\sigma^2}\right)^{p-1/2} & p < \recip{2} \\
\frac{\pi}{32}\left[12 \ln(2) - 1 + 6 \ln(w)\right] & p = \recip{2} \end{cases};\\ 
...
\begin{align}
\Delta E\sub{b}^2 \approx {} & \frac{16}{3}\sqrt{2\pi}G^2m_1^2m_2^2n_\ast\sigma\ln qv_2^2 k \left\{\begin{array}{ll}
...
\left(\dfrac{v_2^2}{2\sigma^2}\right)^{-1/2}\beta\left(\frac{v_2^2}{2\sigma^2};p\right) & \dfrac{v_2^2}{2\sigma^2} > 1\end{array}\right\}\,\delta t.
\end{align}
This can be rolled into one continuous function
\begin{align}
\Delta E\sub{b}^2 \approx {} & \frac{16}{3}\sqrt{2\pi}G^2m_1^2m_2^2n_\ast\sigma\ln qv_2^2 k \left[1 + \left(\frac{v_2^2}{2\sigma^2}\right)^4\right]^{-1}\left\{\left[\dfrac{3}{(1 + p)(2 - p)}\dfrac{v_2^2}{2\sigma^2} - \dfrac{9}{5(3-p)}\left(\dfrac{v_2^2}{2\sigma^2}\right)^2 + \dfrac{9p}{14(7-p)}\left(\dfrac{v_2^2}{2\sigma^2}\right)^3 \right]\right. \nonumber\\*
 & + \left. \left(\frac{v_2^2}{2\sigma^2}\right)^{7/2}\beta\left(\frac{v_2^2}{2\sigma^2};p\right)\right\}\,\delta t\\
 \approx {} & 16\sqrt{2\pi}G^2m_1^2m_2^2n_\ast\sigma\ln qv_2^2 k \gamma\left(\frac{v_2^2}{2\sigma^2};p\right)\,\delta t,
\end{align}
defining the function $\gamma(w;p)$ in the last line. The resulting error (ignoring variation from $\ln qv_2^2$) is less than $3\%$.
...
\begin{equation}
\Delta E\sub{u}^2 \approx \frac{16}{3}\sqrt{2\pi}G^2m_1^2m_2^2n_\ast\sigma\ln qv_2^2 C \Xi\left(\frac{v_2^2}{2\sigma^2}\right)\left[\Xi^2 + \left(\frac{v_2^2}{2\sigma^2}\right)^3\right]^{-1/2}\,\delta t,
\end{equation}
...
\begin{equation}
\Delta E^2 \approx \sum_M \frac{16}{3}\sqrt{2\pi}G^2m_M^2m_2^2n_\ast\sigma\ln qv_2^2 \left\{k_M \gamma\left(\frac{v_2^2}{2\sigma^2};p_M\right) + C_M\Xi\left(\frac{v_2^2}{2\sigma^2}\right)\left[\Xi^2 + \left(\frac{v_2^2}{2\sigma^2}\right)^3\right]^{-1/2}\right\}\,\delta t.
\end{equation}
...
\tau\sub{R} = {} & \left(\frac{m_2v_2^2}{2}\right)^2\frac{\delta t}{\Delta E^2} \\
 \approx {} & \frac{3v_2^4}{16\sqrt{2\pi}G^2n_\ast\sigma\ln qv_2^2} \left(\sum_M m_M^2 \left\{k_M \gamma\left(\frac{v_2^2}{2\sigma^2};p_M\right) + C_M\Xi\left(\frac{v_2^2}{2\sigma^2}\right)\left[\Xi^2 + \left(\frac{v_2^2}{2\sigma^2}\right)^3\right]^{-1/2}\right\}\right)^{-1}.
\end{align}
...
\begin{align}
\overline{v^2} = {} & 4\pi\intd{0}{\infty}{v^4 p_{\mathrm{b},\,M}(v)}{v} \\
 = {} & \frac{4}{\sqrt{\pi}}\sigma^2\frac{3/2 - p_M}{1/2 - p_M}\frac{\Gamma(p_M + 7/2)}{\Gamma(1 + p_M)}\Beta\left(\frac{5}{2},1 + p_M\right),
\end{align}
assuming that $p_M < 1/2$. In the case $p_M = 1/2$ the integral no longer converges, we encounter a logarithmic divergence. This is not a concern, just as with the divergence encountered in \secref{time-scale}, it reflects there being a physical cut-off, in this case there is a maximal velocity. Will will use the speed of light $v\sub{max} = c$: this is a definite hard cut-off. In reality, the maximum velocity would be lower: it could only be reached by a parabolic orbit that grazes the event horizon. 
