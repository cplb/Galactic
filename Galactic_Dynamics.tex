...
To reduce any potential bias, we use the smallest of the calculated widths.\footnote{The distributions were checked to ensure that they did not have anomalously small widths due to a computational error.} The $68$-percentile half-width appears more robust agianst biasing.
...
\begin{center}
   ...
   \subfigure[{Spin magnitude}]{\includegraphics[width=0.4\textwidth]{Fig_mission_sigma_1}} \\
   \subfigure[{Cosine of polar angle}]{\includegraphics[width=0.4\textwidth]{Fig_mission_sigma_1}} \quad
   \subfigure[{Azimuthal angle}]{\includegraphics[width=0.4\textwidth]{Fig_mission_sigma_1}}
...
\left\langle H_{a_\asst}\right\rangle\sub{mission} & = 3.0 \pm 0.4 \units{nats}; \\
...
\end{split}
\end{equation}
using $H_\lambda \def H(p(\lambda)|q(\lambda))$ for brevity. The quoted uncertainties are just the standard errors calculated from the scatter of entropies and do not include any of the other uncertaintites. The typical entropy is about $3 \units{nats}$; this corresponds to an improvement to the precision we know parameters by a factor of approximately $20$.
