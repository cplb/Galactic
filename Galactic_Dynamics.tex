...
\label{eq:Tidal}
\end{equation}
where $R_M$ is the radius of the star~\citep{Hills1975, Rees1988, Kobayashi2004}. Any star on an orbit with $r\sub{p} < r\sub{T}$ will be disrupted in the course of its orbit. Tidal disruption is most significant for MS stars since they are least dense; calculated in this way, only MS stars would be tidally disrupted outside of the MBH's event horizon. The tidal radius defines the cut-off for periapsis of high eccentricity ($e \ga 1$) orbits~\citep{Lightman1977}
