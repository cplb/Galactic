...
The average of the change in energy is more involved. First we will replace the term $\ln\left(qv_{M'}^2\right)$ in $\Delta E^2$ by a suitable average so that it may be moved outside the integral~\citep[chapter 2]{Chandrasekhar1960}. We will assume that it may be replaced by the Coulomb logarithm, which can be approximated as~\citep{Bahcall1976}
\begin{equation}
\ln\left(q\overline{v_2^2}\right) = \ln \Lambda_M \simeq \ln\left(\frac{M_\bullet}{M}\right).
\end{equation}
We will calculate the averages for the bound and unbound contributions individually. In calculating the bound component we must distinguish between the the bound population of field stars and the distribution of test stars over which we are averaging. We will use $M$ and $M'$ respectively for this calculation. The average is
\begin{align}
\overline{\Delta E^2_{\mathrm{b}\,M'}} = {} & 4\pi\intd{0}{\infty}{\Delta E^2\sub{b} v^2 p_{\mathrm{b},2}(v)}{v} \\
 \simeq {} & \sum_M\frac{32}{3}\frac{G^2m_M^2m_{M'}^2n_\ast}{\sigma^2}\ln\left(\Lambda_{M'}\right) k_M \frac{(3/2 - p_{M'})\Gamma(p_{M'} + 7/2)}{\Gamma(p_{M'} + 1)} \nonumber \\* 
 {} & \times \left[ \intd{0}{\sqrt{2}\sigma}{v^2\left(\frac{v^2}{2\sigma^2}\right)^{p_{M'}-2} \frac{3}{(2 - p_M)(1 + p_M)} {_3F_{M'}}\left(-1-p_M,2-p_M,\frac{3}{2};3-p_M,\frac{5}{2};\frac{v^2}{2\sigma^2}\right) \Beta\left(\frac{v^2}{2\sigma^2};3-p_{M'},1+p_{M'}\right)}{v} \right. \nonumber \\* 
...
\begin{align}
\overline{\Delta E^2_{\mathrm{b}\,M'}} \approx {} & \sum_M\frac{32}{3}\frac{G^2m_M^2m_{M'}^2n_\ast}{\sigma^2}\ln\left(\Lambda_{M'}\right) k_M \frac{(3/2 - p_{M'})\Gamma(p_{M'} + 7/2)}{\Gamma(p_{M'} + 1)} \left[ \frac{30 + 36p_M + 25p_M^2 - p_{M'}\left(13 + 15p_M + 7 p_M^2\right) + p_{M'}^2\left(6 + 9p_M + 8p_M^2\right)}{210} \right. \nonumber \\* 
 {} & + \left. \vphantom{\frac{30 + 36p_M + 25p_M^2 - p_{M'}\left(13 + 15p_M + 7 p_M^2\right) + p_{M'}^2\left(6 + 9p_M + 8p_M^2\right)}{210}} \iota \left(p_M,p_{M'}\right) \right] \delta t,
\end{align}
introducing
\begin{equation}
...
\begin{align}
\overline{\Delta E^2_{\mathrm{u}\,M'}} = {} & 4\pi\intd{0}{\infty}{\Delta E^2\sub{u} v^2 p_{\mathrm{b},2}(v)}{v} \\
 \approx {} & \sum_M\frac{32}{3}\frac{G^2m_M^2m_{M'}^2n_\ast}{\sigma^2}\ln\left(\Lambda_{M'}\right) C_M \frac{(3/2 - p_{M'})\Gamma(p_{M'} + 7/2)}{\Gamma(p_{M'} + 1)} \left\{ \intd{0}{\sqrt{2}\sigma}{v^2\left(\frac{v^2}{2\sigma^2}\right)^{p_{M'}-2}\Beta\left(\frac{v^2}{2\sigma^2};3-p_{M'},1+p_{M'}\right)}{v} \right. \nonumber \\* 
 {} & + \left. \intd{\sqrt{2}\sigma}{\infty}{v^2\left(\frac{v^2}{2\sigma^2}\right)^{p_{M'}-2} \Xi\left[\Xi^2 + \left(\frac{v^2}{2\sigma^2}\right)^3\right] \Beta\left(3-p_M,1+p_M\right)}{v} \right\} \delta t;
\end{align}
...
\begin{align}
\overline{\Delta E^2_{\mathrm{u}\,M'}} \approx {} & \sim_M\frac{32}{3}\frac{G^2m_M^2m_{M'}^2n_\ast}{\sigma^2}\ln\left(\Lambda_{M'}\right) C_M \frac{(3/2 - p_{M'})\Gamma(p_{M'} + 7/2)}{\Gamma(p_{M'} + 1)} \left[\nu\left(p_{M'}\right) + \Xi\frac{\Beta\left(3-p_{M'},1+p_{M'}\right)}{2-p_{M'}}{_{M'}F_M}\left(\recip{2},\frac{2-p_{M'}}{3};\frac{}{5-p_{M'}}{3};-\xi^2\right) \right] \delta t,
\end{align}
where
\begin{equation}
\nu(p) = \begin{cases} \recip{1/2 - p}\left[\Beta\left(\dfrac{5}{2},1+p\right) - \Beta\left(3-p,1+p\right)\right] & p < \recip{2} \\
\dfrac{\pi}{96}\left[12 \ln(2) - 5\right]
\end{cases} \; .
\end{equation}
The total relaxation time for a species is
\begin{equation}
\overline{\tau_{\mathrm{R,\,M'}}} = \left(\frac{m_{M'}\overline{v_{M'}^2}}{2}\right)^2\frac{\delta t}{\overline{\Delta E^2_{\mathrm{b}\,M'}} + \overline{\Delta E^2_{\mathrm{u}\,M'}}}.
\end{equation}
