...
\begin{equation}
\Delta E^2(v_1) \approx \frac{8\pi}{3} n(v_1)G^2m_1^2 m_2^2\ln\left(qv_2^2\right)\left\{\begin{array}{lr}\dfrac{v_1^2}{v_2} & v_1 \leq v_2\\ \dfrac{v_2^2}{v_1} & v_1 \geq v_2 \end{array}\right\}\,\dd v_1\delta t.
\end{equation}
...
\begin{align}
\Delta E^2 \approx {} & \frac{16}{3}\sqrt{2\pi}\frac{G^2m_1^2 m_2^2n_\ast}{\sigma^3}\ln\left(qv_2^2\right) \left(\frac{v_2^2}{2\sigma^2}\right) \left[k \frac{3}{(2 - p)(1 + p)}{_3F_2}\left(-1-p,2-p,\frac{3}{2};3-p,\frac{5}{2};\frac{v_2^2}{2\sigma^2}\right) + C\right]\,\delta t,
\end{align}
...
\begin{equation}
\Delta E^2 \approx \frac{16}{3}\sqrt{2\pi}G^2m_1^2 m_2^2n_\ast\sigma\ln\left(qv_2^2\right) \left(\frac{v_2^2}{2\sigma^2}\right)^{-1/2} \left[k\beta\left(\frac{v_2^2}{2\sigma^2};p\right) + C\alpha\left(\frac{v_2^2}{2\sigma^2}\right)\right]\,\delta t,
\end{equation}
where
\begin{align}
...
\beta(w;p) = {} & \begin{cases} \dfrac{\dfrac{3}{1/2 - p}\left[\Beta\left(\dfrac{5}{2},1+p\right) - \dfrac{3w^{p-1/2}}{2(2-p)}\Beta\left(3-p,1+p\right)\right] & p < \recip{2} \\
...
4\pi v^2 p_{\mathrm{b},\,M}(v) \,\dd v = {} & 4\pi v^2 \frac{f_{\mathrm{b},\,M}(v)}{N_{\mathrm{b},\,M}} \,\dd v \\
 = {} & \sqrt{\frac{2}{\pi}} \frac{v^2}{\sigma^3} \frac{\left(3/2 - p_M\right)\Gamma(p_M + 7/2)}{\Gamma(p_M + 1)} k_M \left(\frac{v^2}{2\sigma^2}\right)^{p_M - 3}\left\{\begin{array}{lr}
...
\end{equation}
We will calculate the averages for the bound and unbound contributions individually. In calculating the bound component we must distinguish between the indices of the bound population of field stars and the distribution of test stars over which we are averaging. We will use $p_1$ and $p_2$ respectively for this calculation. The average is
\begin{align}
\overline{\Delta E^2\sub{b,2}} = {} & 4\pi\intd{0}{\infty}{\Delta E^2\sub{b} v^2 p_{\mathrm{b},2}(v)}{v} \\
 \approx {} & \frac{32}{3}\frac{G^2m_1^2m_2^2n_\ast}{\sigma^2}\ln\left(\Lambda) k \frac{(3/2 - p_2)\Gamma(p_2 + 7/2)}{\Gamma(p_2 + 1)}\left\{\intd{0}{\sqrt{2}\sigma}{v^2\left(\frac{v^2}{2\sigma^2}\right)^{p_2-2} \frac{3}{(2 - p_1)(1 + p_1)}{_3F_2}\left(-1-p_1,2-p_1,\frac{3}{2};3-p_1,\frac{5}{2};\frac{v^2}{2\sigma^2}\right) \Beta\left(\frac{v^2}{2\sigma^2};3-p_2,1+p_2\right)}{v} \right. \nonumber \\* 
 {} & + \left.\intd{\sqrt{2}\sigma}{\infty}{v^2\left(\frac{v^2}{2\sigma^2}\right)^{p_2-5/2} \beta\left(\frac{v^2}{2\sigma^2};_1p\right) \Beta\left(3-p_1,1+p_1\right)}{v} \right\}\delta t.
\end{align}
The high velocity integral can be performed without difficulty, but the low velocity piece is more formidable. Progress can be made by making a series expansion in $v^2/2\sigma^2$. Retaining terms to third order approximates the integrand to no worse than $10\%$, with good agreement across most of the integration range. The result may be condensed into a simpler form by approximating it as a quadratic in $p_1$ and $p_2$, which introduces less than $2\%$. After this manipulation
\begin{align}
\overline{\Delta E^2\sub{b,2}} \approx {} & \frac{32}{3}\frac{G^2m_1^2m_2^2n_\ast}{\sigma^2}\ln\left(\Lambda) k \frac{(3/2 - p_2)\Gamma(p_2 + 7/2)}{\Gamma(p_2 + 1)}\left\{\frac{30 + 36p_1 + 25p_1^2 - p_2\left(13 + 15p_1 + 7 p_1^2\right) + p_2^2\left(6 + 9p_1 + 8p_1^2\right)}{210} \right. \nonumner \\* 
 {} & + \left. \iota\left(p_1,p_2\right) \right\}\delta t,
\end{align}
introducing
\begin{equation}
\iota\left(p_1,p_2\right) = \Beta\left(3-p_2,1+p_2\right) \begin{cases} \dfrac{3}{1/2 - p}\left[\dfrac{\Beta\left(5/2,1+p\right)}{2-p_2} - \dfrac{3\Beta\left(3-p_1,1+p_1\right)}{2\left(2-p_1\right)\left(5/2 - p_1 - p_2\right)}\right] & p_1 < \recip{2} \\
\dfrac{\pi}{32}\dfrac{4 + p_2 + 12 \left(2 - p_2\right) \ln(2)}{2-p_2} & p = \recip{2} \end{cases}.
\end{equation}

To calculate the unbound component we will use the exact form for the low velocity component and the approximate form of \eqnref{}
