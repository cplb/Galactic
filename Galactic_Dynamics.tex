...
The information carried by a burst is encoded in its posterior probability distribution. This can be recovered using an MCMC. We ran MCMCs for bursts from the first $100$ of our mission realisations that had periapses $r\sub{p} < 16 r\sub{g}$. There were a total of $96$ interesting bursts ($...$ from BHs and $...$ from NSs) across $63$ missions. Ideally we would use information from all detectable bursts, but this would be computationally expensive and we do not expect to gleam much useful information from orbits with larger periapses.
...
The simplest thing to do is to bin the points and then multiply the numbers in each bin together (dividing by the area of the bin to convert back to a probability density). The question is then what is an appropriate bin size? 

How to multiple: $k$-d trees.

The precision to which a parameter can be constrained may be quantified by the width of the distribution. We shall use the standard deviation $\sigma\sub{SD}$ and the half-width of the $68$-percentile range constructed from one-dimensional $k$-d trees $\sigma_{0.68}$. The widths coincide for Gaussian posteriors.

Possible overestimation. Take smaller due to biasing.\footnote{The distributions were checked to ensure that they did not have anomalously small widths due to a computational error.}

\Figref{Widths} shows the fraction of missions $\mathcal{F}(\sigma > \varsigma)$ that have posterior widths larger than $\varsigma$.
\begin{figure*}
\begin{center}
   \subfigure[{Logarithm of mass}]{\includegraphics[width=0.4\textwidth]{}} \quad 
   \subfigure[{Spin magnitude}]{\includegraphics[width=0.4\textwidth]{}} \\
   \subfigure[{Cosine of polar angle}]{\includegraphics[width=0.4\textwidth]{}} \quad
   \subfigure[{Azimuthal angle}]{\includegraphics[width=0.4\textwidth]{}}
...
Calculated from $k$-d trees. Take larger value, although less sensitive than widths.

The entropies are well correlated with the logarithm of the distribution widths as expected. The cumulative distributions of the fraction of missions with entropies smaller than a given value closely mirror those in \figref{Width} (but the scale on the ordinate axis is now linear). There is a clustering at small entropies; the largest entropies are $\sim ...\units{nats}$.
...
\begin{equation}
\begin{split}
\left\langle H_{\ln(M_\bullet/M_\odot)}\right\rangle\sub{mission} & = \pm \units{nats}; \\
\left\langle H_{a_\asst}\right\rangle\sub{mission} & = \pm \units{nats}; \\
\left\langle H_{\cos\Theta\sub{K}}\right\rangle\sub{mission} & = \pm \units{nats}; \\
\left\langle H_{\Phi\sub{K}}\right\rangle\sub{mission} & = \pm \units{nats},
\end{split}
\end{equation}
using $H_\lambda \def H(p(\lambda)|q(\lambda))$ for brevity. The typical entropy is about $\units{nats}$; this corresponds to ... of approximately $10^{...}$.
